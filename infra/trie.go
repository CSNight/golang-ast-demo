// Package infra Package mux implements a high performance and powerful trie based url path router for Go.
package infra
...
//	// node2.parent == node1
//	// node2 == node3
func (t *Trie) Parse(pattern string, value any) *Node {
...
// Match try to match path. It will return a Matched instance that
...
func (t *Trie) Search(path string) (bool, any, map[string]string) {
	match, err := t.Match(path)
...
func matchNode(parent *Node, segment, path string) (child *Node) {
	...
	for _, child = range parent.segChildren {
		...
		if len(path) == 0 && len(child.optionChildren) == 0 {
			continue
		}
		return child
	}
